create table public.n8n_conversation_state_whatsapp (
  id serial not null,
  ...
  constraint n8n_conversation_state_whatsapp_pkey primary key (id),
  constraint n8n_conversation_state_whatsapp_session_id_key unique (session_id)
) TABLESPACE pg_default;

create index IF not exists idx_n8n_conversation_state_whatsapp_session_id
on public.n8n_conversation_state_whatsapp
using btree (session_id)
TABLESPACE pg_default;

create trigger n8n_conversation_state_whatsapp_updated_at
before update on public.n8n_conversation_state_whatsapp
for each row
